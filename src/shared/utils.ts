...
// Copied from Phaser codebase, because the server can't import phaser
export const Snap = {
    to(value: number, gap: number, start?: number, divide?: boolean): number {
        if (gap === 0) {
            return value
        }
    ...
    ceil(value: number, gap: number, start?: number, divide?: boolean): number {
        if (gap === 0) {
            return value
        }
